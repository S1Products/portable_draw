...
class CanvasDrawer {
  ...
  void drawCanvas(Canvas canvas, Size size, CanvasDef canvasDef,
      [Map<String, OverrideValuesMap>? overrideLayerMap]) {
    drawLayers(canvas, size, canvasDef.layerDefList, canvasDef.scalingDef,
        overrideLayerMap);
  }
  ...
  void drawLayers(
      Canvas canvas, Size size, List<LayerDef> layerDefList, ScalingDef scaling,
      [Map<String, OverrideValuesMap>? overrideLayerMap]) {
  ...
  /// Main drawing method.
  void _draw(
      Canvas canvas, DrawDef drawDef, Map<String, Object>? overrideValueMap) {
  ...
  void _drawText(
    ...
    TextSpan span = TextSpan(
      ...
      text: _getValue(
          overrideValueMap, OverrideValuesMap.ATTRIBUTE_TEXT, textDef.text),
    );
  ...
  void _drawArc(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawArc needs PaintDef in json.');
    }

    RectDef rectDef = _getValue(
        overrideValueMap, OverrideValuesMap.ATTRIBUTE_RECT, draw.rectDef);
    Rect rect = calc.calcScalingRect(rectDef, draw.boxFit);
    canvas.drawArc(
        rect, draw.startAngle, draw.sweepAngle, draw.useCenter, paint);
  }
  ...
  void _drawCircle(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawCircle needs PaintDef in json.');
    }

    Offset offset = calc.calcScalingOffset(draw.offsetDef, draw.boxFit);
  ...
  void _drawDRRect(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawDRRect needs PaintDef in json.');
    }

    RectDef outerRectDef = _getValue(overrideValueMap,
        OverrideValuesMap.ATTRIBUTE_OUTER_RECT, draw.outerRectDef);
    RRect outerRrect =
        calc.calcScalingRRect(outerRectDef, draw.outerRadius, draw.boxFit);

    RectDef innerRectDef = _getValue(overrideValueMap,
        OverrideValuesMap.ATTRIBUTE_INNER_RECT, draw.innerRectDef);
    RRect innerRrect =
        calc.calcScalingRRect(innerRectDef, draw.innerRadius, draw.boxFit);
  ...
  void _drawLine(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawLine needs PaintDef in json.');
    }

    Offset offset1 = calc.calcScalingOffset(draw.offsetDef1, draw.boxFit);
    Offset offset2 = calc.calcScalingOffset(draw.offsetDef2, draw.boxFit);
  ...
  void _drawOval(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawOval needs PaintDef in json.');
    }
  ...
  void _drawPath(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawPath needs PaintDef in json.');
    }
  ...
  void _drawRect(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawRect needs PaintDef in json.');
    }
  ...
  void _drawRrect(
    ...
    if (paint == null) {
      throw ArgumentError.notNull(
          'PaintDef is null. DrawRRect needs PaintDef in json.');
    }
...
// Clip
  ...
  void _clip(Canvas canvas, ClipDef clipDef) {
    switch (clipDef.clipType) {
      case ClipType.Rect:
        _clipRect(canvas, clipDef);
        break;

      case ClipType.RRect:
        _clipRRect(canvas, clipDef);
        break;

      case ClipType.Path:
        _clipPath(canvas, clipDef);
        break;
    }
  }

  void _clipRect(Canvas canvas, ClipDef clipDef) {
    ClipRectDef clipRectDef = clipDef as ClipRectDef;

    Rect rect = calc.calcScalingRect(clipRectDef.rectDef, clipRectDef.boxFit);
    canvas.clipRect(rect,
        clipOp: clipRectDef.clipOp, doAntiAlias: clipRectDef.doAntiAlias);
  }

  void _clipRRect(Canvas canvas, ClipDef clipDef) {
    ClipRRectDef clipRRectDef = clipDef as ClipRRectDef;

    RRect rrect = calc.calcScalingRRect(
        clipRRectDef.rectDef, clipRRectDef.radius, clipRRectDef.boxFit);
    canvas.clipRRect(rrect, doAntiAlias: clipRRectDef.doAntiAlias);
  }

  void _clipPath(Canvas canvas, ClipDef clipDef) {
    ClipPathDef clipPathDef = clipDef as ClipPathDef;
...
// Create Painter
  ...
  /// Create paint brush
  Paint? _createPaint(
      DrawDef drawDef, BoxFit boxFit, Map<String, Object>? overrideValueMap) {
    ...
    paint.color = _getValue(overrideValueMap, OverrideValuesMap.ATTRIBUTE_COLOR,
        Color(paintDef.color));
...
// Utility methods
  ...
  dynamic _getValue<T>(Map<String, dynamic>? overrideValueMap, String attribute,
      T defaultValue) {
